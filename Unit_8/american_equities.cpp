...
int main()
{
	...
	cout << "The password " << passWord << " contains " << countLetters(passWord) << " letters and " << countDigits(passWord) << " digits." << endl;
	return 0;
}
...
{
	int numLetters, numDigits, length, lowercase;

	length = strlen(custPass);
	numLetters = countLetters(custPass);
	numDigits = countDigits(custPass);
	lowercase = islower(*custPass);

	if (numLetters == 4 && numDigits == 6 && length == 10 && lowercase)
		return true;
	else
		return false;
}
